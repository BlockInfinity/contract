...
var matched_ask_ReserveOrderMapping = {};

var ask_reserve_prices = {};
...
function determineReserveAskPrice() {
    ...
    while (!isFound) {
        while (idToOrder[iter_ask_id].price === reserve_ask_price) {
            ...
            appendToDoubleMapping(matched_ask_ReserveOrderMapping, period, owner, {
                volume: volume
            });
    ...
    ask_reserve_prices[period] = reserve_ask_price;
}

var cumBidReserveVol = 0;
var reserve_bid_price;
var matched_bid_ReserveOrderMapping= {};
...
function determineReserveBidPrice() {
    ...
    while (!isFound) {
        while (idToOrder[iter_bid_id].price === reserve_bid_price) {
            ...
            appendToDoubleMapping(matched_bid_ReserveOrderMapping, period, owner, {
                volume: volume
            });
...
function printReserveOrderMatchingResult() {
    ...
    console.log('######################################');
    console.log('\nReserve Price: ' + ask_reserve_prices[period] + ' | Volume (>1000): ' + cumAskReserveVol);
    console.log('\n######################################');
...
function settle(_user, _type, _volume, _period) {
    ...
    var reservePrice = ask_reserve_prices[period];
    var matchingPrice = matchingPriceMapping[period];

    if (_type === 'PRODUCER') {
        if (matched_ask_ReserveOrderMapping[_period][user]) {
            if (!matched_ask_ReserveOrderMapping[_period][user].offeredVolume) {
                console.warn("Position already settled.");
                return false;
            }

            offered = matched_ask_ReserveOrderMapping[_period][user].offeredVolume;
            ...
            matched_ask_ReserveOrderMapping[_period][user] = {};
...
function test_submit_ask_reserve(_users) {
    for (var i = 0; i < _users; i++) {
        ...
        if (new_save_order("ASK", price, volume, owner)) {
...
function test_submit_bid_reserve(_users) {
    for (var i = 0; i < _users; i++) {
        var volume = Math.floor(Math.random() * 300) + 1;
        var owner = Math.floor(Math.random() * _users) + 100;
        var price = Math.floor(Math.random() * 99) + 1;

        if (new_save_order("BID", price, volume, owner)) {
            reserveProviders.push({ id: owner, vol: volume });
        }
    }
}

function test_submit_ask(_users) {
    for (var i = 0; i < _users; i++) {
...
function test_submit_bid(_users) {
    for (var i = 0; i < _users; i++) {
        var volume = Math.floor(Math.random() * 10) + 1;
        var price = 0;
        var owner = Math.floor(Math.random() * _users) + 1;
...
var dex = {
    ...
    matchedBidOrderMapping,
    test_submit_bid: test_submit_bid,
    test_submit_ask: test_submit_ask,
    test_submit_ask_reserve: test_submit_ask_reserve,
    test_settle,
