...
function settleTest(_numberOfUsers, _period) {
  ...
  for (var i = 0; i < matchedBidOrders.length; i++) {
    ...
    //assert.equal(colleteral[matchedBidOrders[i].owner], -matchedBidOrders[i].volume * matchingPriceMapping[period]);
...
function getEnergyBalance() {
  return (sumConsumed[period] + sumReserveConsumed[period]) - (sumProduced[period] + sumReserveProduced[period]);
}

function checkEnergyBalance() {
  if (getEnergyBalance() !== 0) {
    debugger;
