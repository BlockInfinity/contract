...
contract Etherex {
    ...
    Order public minReserveAsk = Order(0,0,0,0,0);
    uint256 public bigProducerMinVolume = 100000;
    ...
    */
    ...
    function matching(){
        ...
        while(currAsk.id != 0 && currBid.id != 0 && currBid.price >= currAsk.price) {
 
            askVolume += currAsk.volume;
            bidVolume+= currBid.volume;
            price = currAsk.price     
        ...
        while(currAsk.id != 0 && currBid.id != 0 bidVolume > 0) {

            //Round robin so that everyone gets something?
            if(currAsk.volume > currBid.volume) {
                //Delete the bid
                matches.push(Match(currBid.volume, price, currAsk.owner, currBid.owner, block.timestamp));
